...
import {
  KeyRound, Mail, Phone, RefreshCcw, Save, Sparkles, UserRound,
} from "lucide-react";
...
const SnookerProfile = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const merged = { ...user, ...updated, name: updated?.name || form.name, username: updated?.username || user?.username, email: updated?.email || form.email };
  ...
  const handleRefreshToken = async () => {
    ...
    if (!refresh) { setError("Refresh token not found."); return; }
    ...
    } catch (e: any) { setError(e?.message || "Error."); } finally { setRefreshing(false); }
  };
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="flex gap-3 justify-end pt-2">
              ...
              <Button type="submit" disabled={saving}
                className="gap-2 bg-violet-600 hover:bg-violet-700 text-white shadow">
